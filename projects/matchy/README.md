...
## Step 4 - Review
 1. [x] What are the different ways you can access properties on objects? // dot notation or bracket nota.
...
## Step 4 - Create
 1. Write a function called `create` that:
  x - Takes 1 parameter, an object
   x- Checks that the object has a `name` property with a length > 0
   x- Checks that the object has a `species` property with a length > 0
